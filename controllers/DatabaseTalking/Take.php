...
spl_autoload_register(static function ($class) {
    $arr = [__DIR__ . '/../../models/goods',
        ...
        __DIR__ . '/../../models/image',
        __DIR__ . '/../../models/invoice',
        './',
    ];
    foreach ($arr as $val) {
        $path = "$val/$class.php";
...
class Take extends Talking {
     ...
     * @return bool|false only if any error happened or  no orderDetails had been found in object $order
     */
    public function takeorder(Orders $order, &$orderNumber = NULL): bool {
        try {
            $customer = $order->getCustomer();
            $address = $customer->getAddress();
            $addressId = random_int(1, 1000000000);
            $customerId = random_int(1, 1000000000);
            $orderId = random_int(1, 1000000000);
            $orderNumber = $orderId;
    ...
    public function updateCustomer($orderId, Customers $customer) {
        // todo : update customer form $orderId variable
        // use the class Customer
    }

    /**
     * @param Book $book
     * insert the book into DB .
     * @return bool|true if the the book inserted successfully else it return false.
     */
